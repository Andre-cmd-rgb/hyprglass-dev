...
Hyprglass V1 is a repository that installs a disciplined Hyprland workstation layer on top of Arch. It is not an ISO.
...
V1 wraps system tools instead of owning system state because Arch already has the right primitives: NetworkManager, BlueZ, ModemManager, PipeWire/WirePlumber, and Hyprland. Hyprglass should coordinate them cleanly, not replace them.

Future ISO/image work can reuse this repository after the install/check path is proven. ISO generation is intentionally out of scope for V1.
